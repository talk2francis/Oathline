...
const capture = (side="BUY", quantity="0.01", quote="7.20"): CapturedHistory => ({capturedAt:"2026-09-05T01:00:05Z",toolName:"spot.allOrders",toolUseId:"history-read",input:{symbol:"BNBUSDT"},response:[{symbol:"BNBUSDT",orderId:123,side,status:"FILLED",executedQty:quantity,cummulativeQuoteQty:quote,time:1788570000000}]});
const authorized = (overrides: Record<string, unknown> = {}): ReceiptEntry[] => chained([
  {ts:"2026-09-05T00:59:58Z",kind:"proposal",toolUseId:"write-1",proposal:{raw:{},toolName:"spot.newOrder",product:"SPOT",symbol:"BNBUSDT",side:"BUY",orderType:"MARKET",quantity:"0.01",price:null,notionalUsdt:"7.20",confidence:"EXACT",...overrides},notionalUsdt:"7.20"},
...
describe("reconciliation",()=>{
  ...
  it("accepts a partial fill because it remains inside the authorised quantity",()=>{ const entries=authorized(); const result=reconcile([capture("BUY","0.005","3.60")],entries,verifyEntries(entries),"2026-09-05T01:01:00Z"); expect(result.rows[0]?.outcome).toBe("MATCHED"); });
  it("diverges when actual quantity exceeds authorised quantity",()=>{ const entries=authorized(); const result=reconcile([capture("BUY","0.02","14.40")],entries,verifyEntries(entries),"2026-09-05T01:01:00Z"); expect(result.rows[0]?.outcome).toBe("DIVERGED"); expect(result.rows[0]?.differences.join(" ")).toContain("exceeds authorised 0.01"); });
  it("accepts BUY limit price improvement",()=>{ const entries=authorized({orderType:"LIMIT",price:"700"}); const result=reconcile([capture("BUY","0.01","6.90")],entries,verifyEntries(entries),"2026-09-05T01:01:00Z"); expect(result.rows[0]?.execution.price).toBe("690"); expect(result.rows[0]?.outcome).toBe("MATCHED"); });
  it("rejects BUY fill worse than the authorised limit",()=>{ const entries=authorized({orderType:"LIMIT",price:"700"}); const result=reconcile([capture("BUY","0.01","7.10")],entries,verifyEntries(entries),"2026-09-05T01:01:00Z"); expect(result.rows[0]?.outcome).toBe("DIVERGED"); expect(result.rows[0]?.differences.join(" ")).toContain("worse than authorised limit 700"); });
