...
export function deriveLedger(entries: ReceiptEntry[], evaluatedAt: string, mode: LedgerState["mode"], client: string): LedgerState {
  ...
  const session = [...entries].reverse().find((entry) => entry.kind === "session_start" && typeof entry.sessionOpenEquityUsdt === "string" && Number.isFinite(Date.parse(entry.ts)) && Date.parse(entry.ts) <= evaluated);
