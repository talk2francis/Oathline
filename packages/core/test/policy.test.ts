...
describe("eleven registered clauses", () => {
  ...
  it("scope.order_types FAIL is a hard denial", () => { const restricted = mandate(); restricted.scope.order_types = ["LIMIT"]; expect(rule(restricted, snapshot(), ledger(), proposal({ orderType: "MARKET" })).outcome).toBe("OUTSIDE_MANDATE"); });
  ...
  it("cooldown FAIL is a hard denial", () => { const ruling = rule(mandate(), snapshot(), ledger({ lastOrderAt: "2026-09-06T14:21:40.000Z" }), proposal()); expect(clause(ruling, "rate.cooldown_seconds")?.text).toContain("below the 300s cooldown"); expect(ruling.outcome).toBe("OUTSIDE_MANDATE"); });
