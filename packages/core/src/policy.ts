...
export const CLAUSE_REGISTRY: readonly ClauseRegistration[] = [
  ...
  { id: "scope.symbols", tier: "STATIC", description: "Symbol is explicitly granted", permittedFrom: "scope.symbols", render: (clause) => clause.text, vectors: { pass: "BNBUSDT", fail: "BTCUSDT" }, check: ({ mandate, proposal }) => membership("scope.symbols", proposal.symbol ?? "UNKNOWN", mandate.scope.symbols) },
...
function freshness(mandate: Mandate, snapshot: Snapshot | null, evaluatedAt: string): { clause: Clause; fresh: boolean } {
  ...
  if (!snapshot) return { fresh: false, clause: { id: "state.max_age_seconds", tier: "SNAPSHOT", result: "UNEVALUABLE", computed: {}, permitted, text: `snapshot is missing; state no older than ${permitted}s is required` } };
  const captured = Date.parse(snapshot.capturedAt); const evaluated = Date.parse(evaluatedAt);
  if (!Number.isFinite(captured) || !Number.isFinite(evaluated) || evaluated < captured) return { fresh: false, clause: { id: "state.max_age_seconds", tier: "SNAPSHOT", result: "UNEVALUABLE", computed: {}, permitted, text: "snapshot age cannot be determined from the observed timestamps" } };
...
function maxOrder(mandate: Mandate, proposal: ProposedAction): Clause {
  if (proposal.notionalUsdt === null) return { id: "budget.max_order_usdt", tier: "STATIC", result: "UNEVALUABLE", computed: {}, permitted: mandate.budget.max_order_usdt, text: "order notional could not be determined" };
...
function drawdown(mandate: Mandate, snapshot: Snapshot | null, ledger: LedgerState, fresh: boolean): Clause {
  ...
  if (!fresh) return { id: "risk.max_session_drawdown_pct", tier: "SNAPSHOT", result: "UNEVALUABLE", computed: {}, permitted, text: "session drawdown cannot be evaluated on missing or stale state" };
  const open = snapshot?.sessionOpenEquityUsdt ?? ledger.sessionOpenEquityUsdt; const current = snapshot?.account.equityUsdt ?? null;
  if (open === null || current === null || cmp(open, "0") <= 0) return { id: "risk.max_session_drawdown_pct", tier: "SNAPSHOT", result: "UNEVALUABLE", computed: {}, permitted, text: "session drawdown requires observed opening and current equity" };
...
function spread(mandate: Mandate, snapshot: Snapshot | null, proposal: ProposedAction, fresh: boolean): Clause {
  ...
  if (!fresh) return { id: "market.max_spread_bps", tier: "SNAPSHOT", result: "UNEVALUABLE", computed: {}, permitted, text: "market spread cannot be evaluated on missing or stale state" };
  const observed = proposal.symbol === null ? null : snapshot?.market[proposal.symbol]?.spreadBps ?? null;
  if (observed === null) return { id: "market.max_spread_bps", tier: "SNAPSHOT", result: "UNEVALUABLE", computed: {}, permitted, text: `market spread for ${proposal.symbol ?? "unknown symbol"} was not observed` };
...
export function rule(mandate: Mandate, snapshot: Snapshot | null, ledger: LedgerState, proposal: ProposedAction): Ruling {
  ...
  else if (clauses.some((clause) => clause.result === "FAIL") && mandate.escalation.hard_violation === "DENY") outcome = "OUTSIDE_MANDATE";
  else if (clauses.some((clause) => clause.result === "UNEVALUABLE" || clause.result === "FAIL")) outcome = "NEEDS_APPROVAL";
