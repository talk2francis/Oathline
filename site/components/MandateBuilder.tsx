...
type Form = { name: string; expiry: string; timezone: string; products: string[]; symbols: string; sides: string[]; orderTypes: string[]; maxOrder: string; maxDaily: string; maxPosition: string; maxOrders: string; cooldown: string; drawdown: string; spread: string; freshness: string; hardViolation: "DENY" | "ASK" };

const initial: Form = { name: "tide-bnb-evening", expiry: "", timezone: "UTC", products: ["SPOT"], symbols: "BNBUSDT", sides: ["BUY", "SELL"], orderTypes: ["MARKET", "LIMIT"], maxOrder: "15", maxDaily: "40", maxPosition: "50", maxOrders: "3", cooldown: "300", drawdown: "2", spread: "20", freshness: "30", hardViolation: "DENY" };
...
function Toggle({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) { return <button type="button" className={`toggle ${active ? "active" : ""}`} onClick={onClick}>{label}</button>; }
...
export function MandateBuilder() {
  ...
  async function sign() {
    try {
      ...
      for (const value of [form.maxOrder, form.maxDaily, form.maxPosition, form.drawdown, form.spread]) if (!/^\d+(?:\.\d+)?$/.test(value)) throw new Error("Money and risk limits must be non-negative decimal strings.");
  ...
  return <div className="mandate-layout">
    <form className="mandate-form" onSubmit={(event) => { event.preventDefault(); void sign(); }}>
      ...
      <fieldset><legend>Scope</legend><span className="field-label">Products</span><div className="toggle-row">{["SPOT", "MARGIN", "FUTURES"].map((value) => <Toggle key={value} label={value} active={form.products.includes(value)} onClick={() => toggle("products", value)} />)}</div><label>Symbols, comma separated<input {...field("symbols")} /></label><span className="field-label">Sides</span><div className="toggle-row">{["BUY", "SELL"].map((value) => <Toggle key={value} label={value} active={form.sides.includes(value)} onClick={() => toggle("sides", value)} />)}</div><span className="field-label">Order types</span><div className="toggle-row">{["MARKET", "LIMIT"].map((value) => <Toggle key={value} label={value} active={form.orderTypes.includes(value)} onClick={() => toggle("orderTypes", value)} />)}</div></fieldset>
      <fieldset><legend>Limits</legend><div className="form-grid"><label>Per order, USDT<input inputMode="decimal" {...field("maxOrder")} /></label><label>Daily gross, USDT<input inputMode="decimal" {...field("maxDaily")} /></label><label>Position, USDT<input inputMode="decimal" {...field("maxPosition")} /></label><label>Orders per day<input inputMode="numeric" {...field("maxOrders")} /></label><label>Cooldown, seconds<input inputMode="numeric" {...field("cooldown")} /></label><label>Drawdown, %<input inputMode="decimal" {...field("drawdown")} /></label><label>Spread, bps<input inputMode="decimal" {...field("spread")} /></label><label>State age, seconds<input inputMode="numeric" {...field("freshness")} /></label></div></fieldset>
      ...
      <button className="button primary sign-button" type="submit" disabled={signing}>{signing ? "Signing locally…" : "Generate signed mandate"}</button>{error && <p className="form-error">{error}</p>}
