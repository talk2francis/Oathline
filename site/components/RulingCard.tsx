...
const checks: Array<{ tone: Tone; mark: string; label: string; text: string }> = [
  ...
  { tone: "permitted", mark: "✓", label: "rate.cooldown_seconds", text: "No prior successful order is recorded; 300s cooldown is available" },
  { tone: "permitted", mark: "✓", label: "risk.max_session_drawdown_pct", text: "441.00 - 438.20 = 2.80 USDT; 0.63% is within 2.00%" },
...
export function RulingCard({ compact = false }: { compact?: boolean }) {
  return (
    <article className={`ruling-card ${compact ? "compact" : ""}`} aria-label="Oathline ruling document">
      <div className="document-kicker"><span>Oathline ruling</span><span>vector #001</span></div>
      ...
      <div className="hash-grid"><span>mandate <code>2f8dba…</code></span><span>snapshot <code>012a33…</code></span><span>proposal <code>e26112…</code></span><span>submission <code>NOT CALLED</code></span></div>
      <div className="document-foot"><span>REFERENCE POLICY VECTOR · 11 CLAUSES</span><span>ADVISORY · LOCAL REPLAY</span></div>
    </article>
  );
}
